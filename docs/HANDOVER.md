...
> セッションをまたいで再開するための現在地まとめ。最終更新: 2026-06-13（**S0〜S13 完了・ライブ確認済み**）。
> **再開時は S14（ゲストコーナー）の仕様作成から**: 設計合意は §10 に記録済み。
> いつものリズム（spec → commit → Confluence ミラー → 実装 → swift test → ライブ確認 → 完了処理）で。
...
`swift test` = **212 件グリーン**。仕様 = `docs/specs/s13-program-generation.md` / Confluence S13=39550977（S12=39223350, S11=39223320, S10=39223298, S9=39092225, S8=38993921, S7=38895618, S6=38731777）。
...
- 残確認: なし。次は S14（ゲストコーナー）の仕様から。
- 保留中の検討課題は §11（**コーナー頭の毎回自己紹介の抑制**［ユーザー要望 2026-06-13、S14 と同時か
  先行の小スライスで］/ Gemma フォールバック）。署名 .app バンドル化は S15 以降の候補。

## 11. 今後の検討課題（今はやらない）
- **コーナー頭の毎回自己紹介・挨拶の抑制**（ユーザー要望 2026-06-13、次スライス候補）:
  S13 のライブ確認で、8 本目のトークでも「みなさんこんにちは！ケイラボAIラジオのお時間なのだ！
  今日のお相手は…」とフルの番組オープニング風に始まることが判明。原因 = 台本生成プロンプトに
  **番組内の位置**（最初のトークか途中か）と**時間帯**を渡しておらず、LLM が毎回初対面の体で書くため。
  対処案:
  - `DialogueScriptGenerator.makeRequest` に「コーナー位置」コンテキストを追加:
    最初のトーク = 「番組最初のコーナー。簡単な挨拶と自己紹介から入ってよい」/
    2 本目以降 = 「**番組の途中**のコーナー。すでに放送中で会話が続いている体で、
    挨拶・自己紹介・番組名の名乗りはせず、いきなりテーマから入る」。
  - 挨拶の時間帯ずれ: 最初のトークの挨拶も themes.yaml の `greetings:`（朝/昼/夜）に合わせる。
    `SeasonPhrases.dateContext` と同様に **timeContext（{greeting} 相当）をプロンプトに注入**
    （`TimePhrases`/`Greetings` は実装済みなので流用）。準備はローリングで数分先行するが、
    挨拶の時間帯精度としては十分。
  - 配線: `CornerEngine.prepare` が位置を知る必要がある → `CornerRunning.prepare` に
    `position`（最初/途中）の引数を追加し、`BroadcastEngine` が plan のトーク番号から渡す。
    お便りコーナーも「途中」扱い（読み上げから入る、自己紹介なし）。
