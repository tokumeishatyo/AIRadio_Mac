...
> セッションをまたいで再開するための現在地まとめ。最終更新: 2026-06-12（**S12 仕様凍結・実装未着手**）。
> **再開時はまず S12 の実装から**: 仕様 = `docs/specs/s12-themed-talk-letters.md`（commit 済み）、
> Confluence ミラー = S12=39223350（チェックリスト付き）。実装 → swift test → ライブ確認
> （S11 残確認も同時に）→ 完了処理、のいつものリズム。
...
| **S12** | **季節コンテキスト + テーマプール + お便りコーナー** | 🚧 仕様凍結済み（39223350）、実装未着手 |
...
- **S12（仕様凍結済み・次にやる）**: 季節コンテキスト注入（「6 月に春めいて」ズレの根治）+
  テーマプール（お酒/料理/スイーツ/旅行/スポーツ/音楽/映画…からランダム）+ お便りコーナー
  （LLM が架空リスナーのお便り生成 → DJ 読み上げ + 感想 → リクエスト曲）。番組は固定 1 周のまま。
...
- 残確認: **S11 の「ニュースのダッキング直後に即発話」の聴取**（次の放送時、S12 のライブ確認と同時で可）。
