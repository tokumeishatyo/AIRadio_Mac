...
> （**S0〜S18 すべてライブ確認済み・クローズ。次は「読み正確化」（未着手）**）。
> **次タスク = 読み正確化（VOICEVOX ユーザー辞書）**: 固有名・難読語の誤読を矯正（例: SEKAI NO OWARI→セカイノオワリ、お家→おうち、栄光の架橋→エイコウノカケハシ）。
> `config/pronunciations.yaml`（表層→カナ読み[+accent]）を起動時に VOICEVOX `/user_dict` へ冪等同期。**全ひらがな化は不可**（アクセント消失で棒読み）。
> 実機で `/user_dict`・`/user_dict_word`(add_user_dict_word: surface/pronunciation(カナ)/accent_type必須int/word_type/priority)・`/import_user_dict` 確認済み・現在空。
> 既存 `VoicevoxTTS.normalizeForSpeech`（波ダッシュ→長音）と同レイヤー。詳細・段階は §10.5 と永続メモリ（airadio-mac-tech-decisions）。
> **仕様駆動で進める**: まず `docs/specs/s19-*.md`（仮）を書いてユーザーレビュー → 実装。優先度: 読み正確化＝中、選曲多様化A/B＝低。
> 横断ルールは `../CLAUDE.md`、仕様は Confluence（下記）。S16〜S18 の採否・設計は §10.5 と永続メモリに記録済み。
...
## 3. 進捗（S0〜S18 完了・ライブ確認済み、次は「読み正確化」未着手）
| スライス | 内容 | 状態 |
|---|---|---|
