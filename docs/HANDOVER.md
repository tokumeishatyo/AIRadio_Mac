...
> （**S13.5 実装完了・ライブ確認待ち**。S0〜S13 はライブ確認済み）。
> **再開時はまず S13.5 の実放送確認**: ①曜日でメインが替わりサブが返す（今日のメインは曜日表 §10/spec §2 で確認）
> ②冒頭トークだけ時刻連動の挨拶＋出演者紹介、他コーナーは「◯時◯分…〈コーナー名〉です」で開始
> ③時報リード文の時刻が再生時点で正確 ④日曜は3人運営。OK なら S14（ゲスト）の仕様作成へ。
...
## 3. 進捗（S0〜S13 完了 + S13.5 実装完了/ライブ確認待ち、次は S14）
| スライス | 内容 | 状態 |
|---|---|---|
...
| S13.5 | 曜日替わりメインDJ + コーナー頭の二層化（挨拶整理・時報リード文）+ つむぎ追加 | 🚧 実装完了・**ライブ確認待ち** |

`swift test` = **236 件グリーン**。仕様 = `docs/specs/s13.5-weekday-main-dj.md` / Confluence S13.5=39419922（S13=39550977, S12=39223350, S11=39223320, S10=39223298, S9=39092225, S8=38993921, S7=38895618, S6=38731777）。
...
- **S13.5（🚧 実装完了・ライブ確認待ち 2026-06-13）**: 曜日替わりメインDJ + コーナー頭の二層化
...
- 残確認: **S13.5 の実放送確認**（曜日替わりメイン・冒頭挨拶・時報リード文の時刻正確性・日曜3人）。
  OK なら S14（ゲストコーナー）の仕様から。
