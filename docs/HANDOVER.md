...
> セッションをまたいで再開するための現在地まとめ。最終更新: 2026-06-12（S8 完了時点）。
...
| S8 | 時刻・日付入りアナウンス + ニュース DJ=青山龍星 + 音量バランス | ✅（実時刻読み上げ確認済み 2026-06-12） |
| **S9** | **候補: メニューバー UI（開始/停止）/ ニュースの LLM 会話化 / 番組の連続ループ（ユーザーと相談して決める）** | ⏭ 次 |

`swift test` = **151 件グリーン**。仕様 = `docs/specs/s8-time-announcements.md` / Confluence S8=38993921（S7=38895618, S6=38731777）。
...
- **音量バランスは config で調整**: 音楽=themes/corners の `volume`（現在 100）、声=tts.yaml の
  `playback_volume`（0〜1、現在 0.65。ユーザーが好みに調整中）。
...
```
メニューバー UI は未実装（S8 候補）。今は `main.swift` のデモ切替で動作確認している。
...
## 10. 次にやる S9 の候補（ユーザーと相談して決める）
- メニューバー UI（NSStatusItem、開始/停止、§3-1 の Task.cancel 配線。S7 の BroadcastEngine をそのまま繋ぐ）
- ニュース・天気の LLM 会話化（S5 の原稿を S6 のコーナーパターンの theme に流し込む）
- 番組の連続ループ / 標準番組構成 13 項目への拡張（新セグメント: 冒頭曲・DJ の気分・お便り等）
