...
> セッションをまたいで再開するための現在地まとめ。最終更新: 2026-06-12（S6 完了時点）。
...
| **S7** | **候補: 番組進行（複数コーナー連結・放送エンジン）/ メニューバー UI / ニュースの LLM 会話化（ユーザーと相談して決める）** | ⏭ 次 |

`swift test` = **119 件グリーン**。仕様 = `docs/specs/s6-gemini-dialogue-corner.md` / Confluence S6=38731777。
...
## 10. 次にやる S7 の候補（ユーザーと相談して決める）
- 番組進行（複数コーナーの連結、標準番組構成 13 項目への接近、放送エンジン）
- メニューバー UI（NSStatusItem、開始/停止、§3-1 の Task.cancel 配線）
