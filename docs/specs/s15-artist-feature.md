...
- **フロー（K=7 のフル形。話者: メイン＝当日先頭、サブ＝以降、全員＝メイン＋サブ）**:
  ...
  | (10) | 締め（発話） | 「{artist}特集でした。」で締める（固定行・準備時に実名展開） | メイン | — |
...
- **発話パートごとに個別生成**: §4/§6 の発話パート（(1) 導入 / 各グループ紹介 / 各感想）を**別々の `DialogueScript`**として
  ...
  - **グループ紹介 (2)(5)(8)**: 当該グループの各曲を順に「曲名／聴きどころ」をまとめて紹介。メイン主導・サブ補足
    ...
    - **会話の連続感（位置依存）**: `ArtistFeaturePart.groupIntro(tracks:index:total:)` に位置を持たせ、`makeArtistFeatureRequest` で出し分ける。
      **1 回目（`index==0`、唯一グループ含む）**は従来どおり自然に曲紹介へ。**2 回目以降（`index>0`）**は**いま進行中の同じ特集を「続ける」つなぎ**
      （「引き続き〈artist〉の曲を」「同じ〈artist〉から、次は」）で前を受け、**アーティスト紹介をやり直さない**。
      **「続いては〈artist〉特集」のように新しく特集が始まる言い方は禁止**（ライブ確認 fix2: 特集途中なのに毎グループで特集が新規開始するように聞こえる不自然さの解消）。
      **最後のグループ（`index==total-1` かつ `index>0`）**は曲数に応じて「最後はこの N 曲」のようにラストである旨を一言添える（縮約で 1〜3 曲）。
      唯一グループ（K=3、`total==1`）は 1 回目扱い。
  - **感想 (4)(7)**: グループを聴いた体での感想・雑談（全員）。(4)=`comment_target_chars`、(7)=`comment_short_target_chars`
    （(7) は (4) より小さい目標。§6/§13）。
  - **(10) 締め**: 「{artist}特集でした。」を含む**固定行**。LLM 生成しない（`DialogueScriptParse` の 4 行下限
    `DialogueScriptGenerator.swift:139` に抵触するため。corners.yaml `outro_line`、既定「以上、{artist}特集でした。」を
    メインの `DialogueLine` として直接積む）。**`{artist}` は準備時にアーティスト名へ展開**（`leadIn` の `{artist}` 置換と同型）。
...
- **`ArtistFeatureEngine`**（fake llm/tts/audio/catalog/spotify/clock）: 7→3+3+1 の分割、縮約（6/5/4/3/≤2）と感想回数、
  ...
  **グループ紹介の位置依存出し分け**＝1 回目は素直・2 回目以降は連続感・最後はラスト明示 /
  締めは固定行「{artist}特集でした。」で**{artist} を実名展開**）、連続再生の曲順・曲数・**各曲 play 後 setVolume**、
...
- corners.yaml（`id: artist_feature`、`format: artist_feature`）:
  ...
  - `outro_line`（固定締め、既定「以上、{artist}特集でした。」。`{artist}` は準備時に実名展開）
...
- 放送中、ゲストコーナーの直後に 1 回だけアーティスト特集が入り、「{時刻}になりました。ここからはアーティスト特集です。
  ...
  「以上、〈アーティスト〉特集でした。」で締める（ユーザー確認）。2 回目以降のグループ紹介は「続いて」等で前を受け、
  最後のグループは「最後はこの N 曲」のようにラストである旨を添える（会話の連続感、ユーザー確認）。
