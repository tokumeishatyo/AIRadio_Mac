...
> （**S15 実装完了・ライブ確認待ち**。S0〜S14 はライブ確認済み）。
> **再開時はまず S15（アーティスト特集）の実放送確認**:
> ① 先にメニュー「アーティスト一覧を生成」を押して `config/artists.yaml` を作る（**出荷時は空＝特集スキップ**）。
>    ジャンル・件数は `config/artist-gen.yaml`（既定 邦楽・100 名、洋楽/クラシック等に変更可・要 Gemini/Spotify）。
> ② 放送し、**ゲストコーナーの直後**に 1 回だけアーティスト特集（口上「{時刻}…ここからはアーティスト特集です。本日は◯◯さんを特集します」
>    → 導入 → 3曲紹介 → 3曲連続 → 感想 → 3曲紹介 → 3曲連続 → 感想(短) → 1曲紹介 → 1曲 → 「アーティスト特集でした。」）。
> ③ 1 放送 1 回（2 回目のニュース後には入らない）。曲が 3 曲未満/プール空なら特集をスキップして放送継続。
> OK なら S16 以降へ。横断ルールは `../CLAUDE.md`、仕様は Confluence（下記）。
...
## 3. 進捗（S0〜S14 完了 + S15 実装完了/ライブ確認待ち、次は S16）
| スライス | 内容 | 状態 |
|---|---|---|
...
| S15 | アーティスト特集（ゲスト直後に 1 回、Spotify top-tracks で 3+3+1、生成ボタン+artist-gen.yaml） | 🚧 実装完了・**ライブ確認待ち** |

`swift test` = **289 件グリーン**。S15 仕様 = `docs/specs/s15-artist-feature.md` / Confluence S15=39419951。S14 仕様 = `docs/specs/s14-guest-corner.md` / Confluence S14=39485449。
...
- 残確認: **S15 の実放送確認**（上記 §冒頭の手順。先に生成ボタンでプール作成 → ゲスト直後に特集 1 回・口上・3+3+1・締め）。
  S13.5 の日曜3人運営はテスト担保・実放送は日曜に確認できれば。
- **語尾混線 fix（commit `ca08625`、push 済み）**: 日曜3人（ずんだもんメイン）で春日部つむぎがメインの「のだ」に語尾伝染していた
  ...
  アーティスト特集の両方）＋ djs.yaml で tsumugi の語尾を明記。**LLM 挙動の変更なので 3 人編成（日曜 or 強語尾メイン）で要再確認**。
