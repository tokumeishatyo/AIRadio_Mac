...
> （**S0〜S18 ＋ S19a すべてライブ確認済み・クローズ。次は「S19b 読み正確化②＝アーティスト読み自動登録」（未着手）**）。
> **次タスク = S19b（アーティスト読み自動登録）**: 「アーティスト一覧を生成」時にカナ読みも併出させ `artists.yaml` に保存 → S19a の辞書同期（`VoicevoxUserDict.mergedEntries`）に合流。
> `ArtistProfile.reading` 追加＋生成の出力契約をタブ区切り化（`名前<TAB>カナ読み`）＋ parseEntries の逸脱耐性（タブ無し＝名前のみ／最初のタブのみ分割／非カタカナ読みは捨てる）。
> **空白英字バンド名は VOICEVOX 辞書では効かない**（実機確定。SEKAI NO OWARI＝棒読み）→通称カナ運用で割り切り（ユーザー判断）。根本対応＝合成前テキスト置換層は将来検討。
> 詳細な申し送りは `docs/specs/s19a-pronunciation-dictionary.md` §12、実機教訓は §8 と永続メモリ（airadio-mac-tech-decisions）。
> **仕様駆動で進める**: まず `docs/specs/s19b-*.md` を書いてユーザーレビュー → 実装。優先度: S19b＝中、選曲多様化A/B＝低。
> 横断ルールは `../CLAUDE.md`、仕様は Confluence（下記）。S16〜S19a の採否・設計は §10.5 と永続メモリに記録済み。
...
## 3. 進捗（S0〜S18 ＋ S19a 完了・ライブ確認済み、次は S19b＝アーティスト読み自動登録 未着手）
| スライス | 内容 | 状態 |
|---|---|---|
...
`swift test` = **342 件グリーン**。S19a 仕様 = `docs/specs/s19a-pronunciation-dictionary.md`（Confluence S19a=39616579）。S18 仕様 = `docs/specs/s18-station-journal.md`。S17 仕様 = `docs/specs/s17-daily-context.md`。S16 仕様 = `docs/specs/s16-mood-and-theme-leadin.md`。S15 仕様 = `docs/specs/s15-artist-feature.md` / Confluence S15=39419951。S14 仕様 = `docs/specs/s14-guest-corner.md` / Confluence S14=39485449。
...
- **S19b（読み正確化②＝アーティスト読み自動登録、優先度 中・次タスク）**: 「アーティスト一覧を生成」時にカナ読みも併出させ `artists.yaml` に保存 → S19a の同期に合流。
  `ArtistProfile.reading` 追加＋生成出力契約をタブ区切り化＋parseEntries 逸脱耐性（タブ無し＝名前のみ／非カタカナ読みは捨てる）。申し送り詳細は s19a spec §12。既存 `VoicevoxTTS.normalizeForSpeech` と同レイヤー。
