...
## 3. 進捗（S0〜S16 完了・ライブ確認済み + S17 実装完了/ライブ確認待ち）
| スライス | 内容 | 状態 |
|---|---|---|
...
| S17 | DailyContext 拡張（曜日名 + 記念日の軽重 high/low）。`DailyCalendar` + `config/calendar.yaml`、CornerEngine/ArtistFeatureEngine に注入 | 🚧 実装完了・**ライブ確認待ち** |

`swift test` = **309 件グリーン**。S17 仕様 = `docs/specs/s17-daily-context.md`。S16 仕様 = `docs/specs/s16-mood-and-theme-leadin.md`。S15 仕様 = `docs/specs/s15-artist-feature.md` / Confluence S15=39419951。S14 仕様 = `docs/specs/s14-guest-corner.md` / Confluence S14=39485449。
...
- **S17** 🚧 実装完了・ライブ確認待ち（2026-06-14、309テスト、spec=`docs/specs/s17-daily-context.md`）: DailyContext 拡張
