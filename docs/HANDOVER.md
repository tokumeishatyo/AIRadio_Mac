...
> （**S0〜S19b ＋ S20(.app化) すべてライブ確認済み・クローズ。次の機能スライスは「選曲多様化 A/B（反復回避・リングバッファ）」（未着手・優先度 低）**）。
> **配布（S20）**: `sh scripts/make-app.sh` で `dist/ケイラボAIラジオ/`（.app＋config の1フォルダ・アドホック署名）を生成 → **ダブルクリック起動**（初回のみ右クリック→開く）。config は .app と同じ階層から解決（`configPath`/`ConfigLocation`、dev `swift run` は ./config のまま）。「Spotify にログイン」メニューあり。
> **運用フェーズ（実質完成・S0〜S20 クローズ）**: 以後は実際に使ってみて不具合・追加要望が出たら再開する。再開は**仕様駆動**（spec→レビュー→実装→`swift test`→ライブ確認→commit、§9）で 1 スライスずつ。
> 既知の機能バックログは **選曲多様化 A/B（優先度 低）** のみ＝通常コーナーの選曲が定番に固定する（原因＝`SongPicker` がステートレス・先頭の再生可能曲を機械採用。詳細 §10.5）。
...
> Confluence 仕様ミラーは **S19b まで**（**S20 はユーザー判断でミラー省略**＝SoT は `docs/specs/s20-app-bundle.md`）。要件定義は v1.3（37519362）、CLAUDE.md ミラー同期済み。
> 記事を **Qiita / Zenn に公開済み**（`../articles/`）、GitHub リポジトリも**公開済み**（`github.com/tokumeishatyo/AIRadio_Mac`、機密混入なしを履歴スキャンで確認済み）。
...
## 3. 進捗（S0〜S19b ＋ S20(.app化) 完了・ライブ確認済み、次の機能は 選曲多様化 A/B 未着手・優先度 低）
| スライス | 内容 | 状態 |
|---|---|---|
...
| S20 | .app バンドル化（ダブルクリック起動）。config パス解決（`ConfigLocation`(Infra純ロジック)＋`AppPaths.configPath()`、.app は同階層 config/・dev は ./config・`AIRADIO_CONFIG_DIR` 上書き可）。「Spotify にログイン」メニュー追加（PKCE 再利用）。`scripts/make-app.sh`（アドホック署名・1フォルダ自己完結・`--no-secrets` 可） | ✅（ライブ確認済み 2026-06-14、commit cd1931a） |

`swift test` = **357 件グリーン**。S20 仕様 = `docs/specs/s20-app-bundle.md`。S19b 仕様 = `docs/specs/s19b-artist-reading.md`（Confluence S19b=39420033）。S19a 仕様 = `docs/specs/s19a-pronunciation-dictionary.md`（Confluence S19a=39616579）。S18 仕様 = `docs/specs/s18-station-journal.md`。S17 仕様 = `docs/specs/s17-daily-context.md`。S16 仕様 = `docs/specs/s16-mood-and-theme-leadin.md`。S15 仕様 = `docs/specs/s15-artist-feature.md` / Confluence S15=39419951。S14 仕様 = `docs/specs/s14-guest-corner.md` / Confluence S14=39485449。
...
sh scripts/make-app.sh                          # 配布: .app バンドル化（dist/ケイラボAIラジオ/。ダブルクリック起動。S20。--no-secrets 可）
```
...
## 11. 今後の検討課題（今はやらない）
- **Developer ID 署名・notarization（他人配布）**（要件§11 後回し）: **S20 でアドホック署名の .app（このMac専用・ダブルクリック起動・1フォルダ自己完結）は実装済み**（`scripts/make-app.sh`）。
  他人へ配布してクリーンなダブルクリック（Gatekeeper 無警告）にするには Apple Developer Program（年$99）＋ notarization が必要。将来。
