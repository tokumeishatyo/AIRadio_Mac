...
> セッションをまたいで再開するための現在地まとめ。最終更新: 2026-06-13
> （**「曲が途中で終わる」解決: 真因は corners.yaml の play_seconds=60。全曲フル再生化 + 終端検知強化**）。
> **再開時はまず実放送 1 周の確認**: 冒頭曲・コーナー締め曲がすべて最後まで流れ、コンソールの
> 「♪ 曲終了（検知: …）」が reachedEnd / stopped / positionStalled のいずれかであること
> （**trackChanged / timedOut が出たら異常** → そのログを採取。`AIRADIO_SPOTIFY_LOG=1` で詳細化）。
> OK なら S13（コーナー数駆動の番組生成、§10 に設計合意済み）の仕様から。
...
## 3. 進捗（S0〜S12 完了、次は途中切り fix の実放送確認 → S13）
| スライス | 内容 | 状態 |
|---|---|---|
...
| fix | フル再生の終端検知強化（stale 耐性）+ 全コーナー曲フル再生化 | ✅ コード完了。**実放送 1 周の確認は次回** |

`swift test` = **205 件グリーン**。仕様 = `docs/specs/s12-themed-talk-letters.md` / Confluence S12=39223350（S11=39223320, S10=39223298, S9=39092225, S8=38993921, S7=38895618, S6=38731777）。
- **S12**: コーナーは corners.yaml の `themes:`（プール）から準備のたびにランダムにテーマを選ぶ
  ...
  → ③台本生成（読み上げ + 感想 + 曲振り）。番組は OP → song → talk → **letter** → news → ED の
  6 セグメント。進行ログに「テーマ: / お便り: ラジオネーム」を出力。
...
- **S13（設計合意済み）**: コーナー数駆動の番組生成 + ローリング先行準備 + ED ボタン + 番組の長さ UI。
  - 番組 = コーナー数 N（**トーク系のみを数える**。OP/ED/お便り/ニュースは含めない）から決定論的に生成:
    トーク 2 つごとに「お便り → ニュース」を自動挿入、最後に ED。N は**最低 10〜エンドレス**、
    メニューのサブメニュー「番組の長さ」（10/20/30/エンドレス、UserDefaults 保存、program.yaml に既定値）。
  - **端数処理**: 奇数の端数トークの後はお便りを挟まず ED へ。
  - **エンドレス時は ED なし**（停止 = 既存の「放送を停止」で即静寂）。
  - **「ED で終了」ボタン**（放送中メニュー）: 押すと「現在再生中のセグメント + すでに準備が完了している
    直後のトークコーナーまで流したら、**お便り/ニュース含む残りをすべて飛ばして ED**」で優雅に締める。
  - エンジンは「全部先行準備」から「次の 1〜2 セグメントを再生中に準備するローリング方式」へ進化
    （ED 割り込みは『窓の外を捨てる』操作として実装）。
- **S14**: ゲストコーナー（1 放送 1 回、トーク枠を差し替え）。VOICEVOX の非レギュラーキャラ
  （実装時に /speakers で実在確認）からテーマに合うゲストを選び、DJ 二人 + ゲストの 3 人会話。
  `DialogueScriptGenerator` は既に任意人数の名前でパース可能。
- **S15**: アーティスト特集（1 放送 1 回）。アーティスト選定 → 5 曲程度まとめてプレフライト →
  そのアーティストへの思いを語る → 連続再生（曲間の曲名コール有無は仕様時に決める）。
- 残確認: **実放送 1 周**（冒頭曲・コーナー締め曲がすべてフル再生で最後まで流れ、
  「♪ 曲終了（検知: …）」が reachedEnd / stopped / positionStalled であること）。
  S11 のダッキング直後即発話は聴取確認済み（2026-06-12）。
