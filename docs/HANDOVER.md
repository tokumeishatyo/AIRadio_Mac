...
> セッションをまたいで再開するための現在地まとめ。最終更新: 2026-06-12（**S12 完了 + フル再生途中切り fix**）。
> **再開時はまず実放送で途中切り fix の確認**: 冒頭曲が 3 分超の曲でも最後まで流れること
> （切れる場合は従来 3 分すぎで切れていた。`AIRADIO_SPOTIFY_LOG=1` で全 Spotify 呼び出しを
> ログできる）。確認できたら S13（コーナー数駆動の番組生成、§10 に設計合意済み）の仕様から。
...
| fix | フル再生の途中切り（stale 曲長）+ 曲終了後のデッドエア解消 | ✅ コード + trackwatch 実機確認済み。**実放送での確認は次回** |

`swift test` = **203 件グリーン**。仕様 = `docs/specs/s12-themed-talk-letters.md` / Confluence S12=39223350（S11=39223320, S10=39223298, S9=39092225, S8=38993921, S7=38895618, S6=38731777）。
...
- 残確認: **フル再生途中切り fix の実放送確認**（冒頭曲が 3 分超でも最後まで流れること。次の放送時）。
