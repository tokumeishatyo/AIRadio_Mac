...
## 3. 進捗（S0〜S15 完了・ライブ確認済み、次は S16 未着手）
| スライス | 内容 | 状態 |
|---|---|---|
...
`swift test` = **296 件グリーン**。S15 仕様 = `docs/specs/s15-artist-feature.md` / Confluence S15=39419951。S14 仕様 = `docs/specs/s14-guest-corner.md` / Confluence S14=39485449。
...
- **S16**: ③DJの今日の気分（OP後の最初の会話のフリに「今日の気分」を持ってくる＝冒頭トーク導入の拡張）＋
  常設コーナーのリード文をテーマ宣言化（「ここからはフリートークのコーナーです」→「ここからは◎◎について話そうと思います」。
  free_talk の `lead_in` に `{theme}` を足し、ゲストの `{theme}` と同じ準備時展開。**別テンプレ建ては不要**）。
