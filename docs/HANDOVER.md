...
> （**S0〜S15 すべてライブ確認済み・クローズ。次は S16（未着手）**）。
> **S15 アーティスト特集は完了**: 構造・曲被りfix・語尾fix・会話の自然さ改善3点＋fix2 をすべてライブ確認済み（2026-06-14）、正規 config 復元・コミット/プッシュ済み。
> 特集の流れ（参考）: ゲストコーナー直後に 1 回だけ → 口上「{時刻}…ここからはアーティスト特集です。本日は◯◯さんを特集します」
> → 導入 → 3曲紹介 → 3曲連続 → 感想 → 3曲紹介(**「引き続き◯◯の曲を」で連続**) → 3曲連続 → 感想(短) → 1曲紹介(**「最後はこの1曲」**) → 1曲 → 「**以上、◯◯特集でした。**」。
> 1 放送 1 回（2 回目のニュース後には入らない）。曲 3 未満/プール空はスキップして放送継続。プールはメニュー「アーティスト一覧を生成」で作成（出荷時は空）。
> **次は S16（ロードマップ未確定）。着手前にユーザーと内容合意を。** 横断ルールは `../CLAUDE.md`、仕様は Confluence（下記）。
...
## 3. 進捗（S0〜S15 完了・ライブ確認済み + S16 実装完了/ライブ確認待ち）
| スライス | 内容 | 状態 |
|---|---|---|
...
| S16 | DJの今日の気分（冒頭 greeting に一言）+ 常設コーナーのテーマ宣言リード文（free_talk lead_in を {theme} 化） | 🚧 実装完了・**ライブ確認待ち** |

`swift test` = **297 件グリーン**。S16 仕様 = `docs/specs/s16-mood-and-theme-leadin.md`。S15 仕様 = `docs/specs/s15-artist-feature.md` / Confluence S15=39419951。S14 仕様 = `docs/specs/s14-guest-corner.md` / Confluence S14=39485449。
...
- **S16** 🚧 実装完了・ライブ確認待ち（2026-06-14、297テスト、spec=`docs/specs/s16-mood-and-theme-leadin.md`）: ③DJの今日の気分
  ...
  残: 実放送で「冒頭の今日の気分の一言」「テーマ宣言リード文」を聴取確認。
- **S17**: DailyContext 拡張（`SeasonPhrases` の季節+日付に**曜日名・記念日**を追加注入）。記念日は
  `config/anniversaries.yaml`（手書き・日付→名前+`significance`）。**軽重**: `high`(祝日級)=全コーナーのプロンプトに
  「今日は〇〇（祝日）、番組を通して織り込む」と強注入（フリーコーナーにも波及）／`low`(軽い暦)=「軽く触れる程度・深入りしない」
  弱注入／なし=従来どおり。プロンプト注入のみ（テーマエンジンは触らない）。
- **S18**: ステーション・ジャーナル（長期記憶）。番組終了時にハイライトを LLM 要約 → `config/journal.local.yaml`
  （gitignore）に永続化 → 次回起動時に注入。**週次リセット**（週を紐づけ、放送開始時に「今が記録済みの週と違えば破棄」＝
  日曜まで貯めて月曜の最初の放送で前週分を捨てる）＋**ファイル人為削除で即クリア**の二段構え。
- **読み正確化（どこでも・優先度 中）**: VOICEVOX ユーザー辞書で誤読矯正。実機で `/user_dict`・`/user_dict_word`・
  `/import_user_dict` 確認済み（現在空）。`config/pronunciations.yaml`（表層→カナ読み[+accent]）を起動時に冪等同期。
  例: SEKAI NO OWARI→セカイノオワリ、お家→オウチ、栄光の架橋→エイコウノカケハシ。**全ひらがな化は不可**（アクセント消失で棒読み）。
  アーティスト名は生成時に読みも出させ自動登録する拡張余地。既存 `VoicevoxTTS.normalizeForSpeech` と同レイヤー。
