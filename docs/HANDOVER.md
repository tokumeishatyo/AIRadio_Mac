...
> （**S14 実装完了・ライブ確認待ち**。S0〜S13.5 はライブ確認済み）。
> **再開時はまず S14 の実放送確認**: ①最初のニュース直後に 1 回だけゲストコーナー（口上「{時刻}…次はゲストコーナーです。
> 本日は◯◯さんを迎えて、◯◯について熱く語ってもらいます」→ 全員で会話・ゲストは専門家役 → お礼 → 曲）
> ②ゲストはレギュラー以外から・声も替わる・あんこもんは語尾「もん」 ③ゲストは 1 放送 1 回（2 回目のニュース後には入らない）。
> OK なら S15（アーティスト特集）の仕様作成へ。横断ルールは `../CLAUDE.md`、仕様は Confluence（下記）。
...
## 3. 進捗（S0〜S13.5 完了 + S14 実装完了/ライブ確認待ち、次は S15）
| スライス | 内容 | 状態 |
|---|---|---|
...
| S14 | ゲストコーナー（最初のニュース直後に 1 回、ランダムゲスト＝テーマ専門家）+ ゲスト 10 名 | 🚧 実装完了・**ライブ確認待ち** |

`swift test` = **261 件グリーン**。S14 仕様 = `docs/specs/s14-guest-corner.md` / Confluence S14=39485449。
...
- **S14（🚧 実装完了・ライブ確認待ち 2026-06-13）**: ゲストコーナー（最初のニュース直後に 1 回）。
  ※当初「トーク枠をランダムに差し替え／テーマに合うゲスト」だったが、ユーザー判断で
  **「最初のニュース直後に固定・ゲストはランダム選定＋テーマ専門家の建付け」**に変更（spec §3 注記）。
  詳細 = `docs/specs/s14-guest-corner.md`（§3 に要約）。
- **S15**: アーティスト特集（1 放送 1 回）。アーティスト選定 → 5 曲程度まとめてプレフライト →
  そのアーティストへの思いを語る → 連続再生（曲間の曲名コール有無は仕様時に決める）。
- 残確認: **S14 の実放送確認**（最初のニュース直後にゲスト 1 回・口上・専門家会話・あんこもん語尾）。
  OK なら S15 の仕様から。S13.5 の日曜3人運営はテスト担保・実放送は日曜に確認できれば。
